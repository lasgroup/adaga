...
class StatisticalTest(object):
    ...
    def _compute_cov_alt(self, model_0, model_1) -> tf.Tensor:
        ...
        """
        k_uf = model_0['variances'][0, :, :] * tf.transpose(model_0['t'])

        h_uf = model_1['variances'][0, :, :] * tf.transpose(model_1['t'])

        sigma_sq = model_0['likelihood_variances'][0, :, :]
        xi_sq = model_1['likelihood_variances'][0, :, :]
        alpha = 1 / sigma_sq + 1 / xi_sq

        d = alpha * sigma_sq * tf.eye(model_0['t'].shape[1], dtype=tf.float64) + alpha * tf.matmul(k_uf, k_uf, transpose_b=True) - (1 / sigma_sq) * tf.matmul(k_uf,
                                                                                                                   k_uf,
                                                                                                                   transpose_b=True)
        print("d", d)
        d_inv_k_uf = tf.linalg.solve(d, k_uf)
        a_inv = 1.0 / alpha * (tf.eye(model_0['t'].shape[1], dtype=tf.float64) + (1 / sigma_sq) * tf.matmul(k_uf,
                                                                                                                   d_inv_k_uf,
                                                                                                                   transpose_a=True))

        c = xi_sq * tf.eye(model_0['t'].shape[1], dtype=tf.float64) + tf.matmul(h_uf, h_uf, transpose_b=True) - (1 / xi_sq) * tf.matmul(h_uf,
    ...
    def _compute_single_expert_inv_covariance(self, model) -> tf.Tensor:
        ...
        """
        k_uf = model['variances'][0, :, :] * tf.transpose(model['t'])
        variance = model['likelihood_variances'][0, :, :]
        sigma = tf.sqrt(model['likelihood_variances'][0, :, :])
        A = k_uf / sigma
        AAt = tf.matmul(A, A, transpose_b=True)
        B = AAt + tf.eye(model['t'].shape[1], dtype=tf.float64)
