...
from utils.GP_approx_risk_minimization import GPApproxRiskMinimization
...
class AdaptiveRegionalization(object):
    ...
    def _create_expert(self, window, new: bool, x_mean=None, x_std=None, y_mean=None, y_std=None):
        ...
        y -= y_mean
        y /= y_std
        expert = GPApproxRiskMinimization(y, x, state_normalization=False, QFF_approx=self.n_feat, new_expert=new)
        expert.build_model()
    ...
    def regionalize(self):
        ...
        while True:
            ...
            with session:
                ...
                _, curr_lth, curr_kern_var, curr_lk_var = model_current_expert.train(session)
            ...
            with session:
                ...
                if min(end, self.x[-1, 0]) - start > self.min_window_size + 3 > end - self.x[-1, 0]:
                    ...
                    _, new_lth, new_kern_var, new_lk_var = model_new_expert.train(session)
                    ...
                    statistical_test = StatisticalTest(model_current_expert_dict, model_new_expert_dict, self.delta, self.n_feat)
